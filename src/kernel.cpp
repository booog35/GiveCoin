...
static bool SelectBlockFromCandidates(vector <pair<int64_t, uint256>> &vSortedByTimestamp,
                                      ...
                                      int64_t nSelectionIntervalStop, uint256 nStakeModifierPrev,
...
bool ComputeNextStakeModifier(const CBlockIndex *pindexPrev, uint256 &nStakeModifier, bool &fGeneratedStakeModifier) {
    ...
    LogPrint("stakemodifier", "ComputeNextStakeModifier: prev modifier=%s time=%s\n", nStakeModifier.ToString(),
             DateTimeStrFormat(nModifierTime));
    ...
    for (int nRound = 0; nRound < min(64, (int) vSortedByTimestamp.size()); nRound++) {
        ...
        LogPrint("stakemodifier", "ComputeNextStakeModifier: selected round %d stop=%s height=%d bit=%d\n", nRound,
                 DateTimeStrFormat(nSelectionIntervalStop), pindex->nHeight, pindex->GetStakeEntropyBit());
    }
...
static bool GetKernelStakeModifier(uint256 hashBlockFrom, uint256 &nStakeModifier, int &nStakeModifierHeight,
...
static bool CheckStakeKernelHashV2(CBlockIndex *pindexPrev, unsigned int nBits, unsigned int nTimeBlockFrom,
    ...
    if (fPrintProofOfStake) {
        ...
        LogPrintf(
                "CheckStakeKernelHash() : check modifier=%s nTimeBlockFrom=%u nTimeTxPrev=%u nPrevout=%u nTimeTx=%u hashProof=%s\n",
                nStakeModifier.ToString(),
    ...
    if (fDebug && !fPrintProofOfStake) {
        LogPrintf("CheckStakeKernelHash() : using modifier %s at height=%d timestamp=%s for block from timestamp=%s\n",
                  ...
                  DateTimeStrFormat(nTimeBlockFrom));
        LogPrintf(
                "CheckStakeKernelHash() : pass modifier=%s nTimeBlockFrom=%u nTimeTxPrev=%u nPrevout=%u nTimeTx=%u hashProof=%s\n",
                nStakeModifier.ToString(),
                nTimeBlockFrom, txPrev.nTime, prevout.n, nTimeTx,
                hashProofOfStake.ToString());
    }
...
bool CheckProofOfStake(CBlockIndex *pindexPrev, const CTransaction &tx, unsigned int nBits, uint256 &hashProofOfStake,
    ...
    if (!VerifySignature(txPrev, tx, 0, SCRIPT_VERIFY_NONE, 0))
        return tx.DoS(100,
                      error("CheckProofOfStake() : VerifySignature failed on coinstake %s", tx.GetHash().ToString()));
...
CheckKernel(CBlockIndex *pindexPrev, unsigned int nBits, int64_t nTime, const COutPoint &prevout, int64_t *pBlockTime) {
    ...
    return CheckStakeKernelHash(pindexPrev, nBits, block, txindex.pos.nTxPos - txindex.pos.nBlockPos, txPrev, prevout,
