...
void CoinControlDialog::sortView(int column, Qt::SortOrder order) {
    ...
    ui->treeWidget->header()->setSortIndicator(
            (sortColumn == COLUMN_AMOUNT_INT64 ? COLUMN_AMOUNT : (sortColumn == COLUMN_POTENTIALSTAKE_INT64
                                                                  ? COLUMN_POTENTIALSTAKE : (sortColumn ==
                                                                                             COLUMN_AGE_INT64
                                                                                             ? COLUMN_AGE
                                                                                             : sortColumn))),
            sortOrder);
...
void CoinControlDialog::headerSectionClicked(int logicalIndex) {
    ...
    {
        ui->treeWidget->header()->setSortIndicator((sortColumn == COLUMN_AMOUNT_INT64 ? COLUMN_AMOUNT : sortColumn),
                                                   sortOrder);
    } else {
        ...
        if (sortColumn == logicalIndex)
            ...
            sortOrder = ((sortColumn == COLUMN_AMOUNT_INT64 || sortColumn == COLUMN_DATE ||
                          sortColumn == COLUMN_CONFIRMATIONS || sortColumn == COLUMN_AGE_INT64 ||
                          sortColumn == COLUMN_POTENTIALSTAKE_INT64) ? Qt::DescendingOrder
                                                                     : Qt::AscendingOrder); // if amount,date,conf then default => desc, else default => asc
        }
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog) {
    ...
    foreach(
    const qint64 &amount, CoinControlDialog::payAmounts)
    {
        nPayAmount += amount;
    ...
    if (nQuantity > 0) {
        ...
        if (pwalletMain->fSplitBlock) {
            nPayFee = COIN /
                      1000; // make fee more expensive if using splitblock, this avoids having to calculate fee based on multiple vouts
        }
...
void CoinControlDialog::updateView() {
    ...
    coins, mapCoins)
    {
        ...
        const COutput &out, coins.second)
        {
            ...
            // potential stake
            double nPotentialStake = (double) nBlockSize * (GetCoinYearReward(nBestHeight) / CENT / 100) *
                                     ((double) nAge / (86400) / 365);
            itemOutput->setText(COLUMN_POTENTIALSTAKE, QString::number(nPotentialStake, 'f', 2));
            itemOutput->setText(COLUMN_POTENTIALSTAKE_INT64,
                                strPad(QString::number((int64_t) nPotentialStake), 16, " "));
            ...
            double nEstimateTime =
                    (double) GetTargetSpacing(nBestHeight) * ((double) nNetworkWeight / ((double) nBlockSize * COIN));
        ...
        if (treeMode) {
            ...
            itemWalletAddress->setText(COLUMN_POTENTIALSTAKE,
                                       BitcoinUnits::formatAge(nDisplayUnit, nPotentialStakeSum));
            itemWalletAddress->setText(COLUMN_POTENTIALSTAKE_INT64,
                                       strPad(QString::number(nPotentialStakeSum), 20, " "));
