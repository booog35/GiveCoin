...
SendCoinsDialog::SendCoinsDialog(QWidget *parent) :
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a GiveCoin address (e.g. HG6jDsuUnmmKEiNjVbRmG5vRpMvzUycoaR)"));
...
void SendCoinsDialog::on_sendButton_clicked() {
    ...
    const SendCoinsRecipient &rcp, recipients)
    {
        if (!model->getSplitBlock()) {
            ...
            formatted.append(tr("<b>%1</b> to %2 (%3)").arg(
                    BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), rcp.amount),
...
#if QT_VERSION < 0x050000
            formatted.append(tr("<b>%1</b> in %4 blocks of %5 each to %2 (%3)?").arg(
    ...
    QMessageBox::StandardButton retval = QMessageBox::question(this, tr("Confirm send coins"),
                                                               tr("Are you sure you want to send %1?").arg(
                                                                       formatted.join(tr(" and "))),
...
void SendCoinsDialog::splitBlockLineEditChanged(const QString &text) {
    double nAfterFee = ui->labelCoinControlAfterFee->text().left(
            ui->labelCoinControlAfterFee->text().indexOf(" ")).toDouble();
    double nSize = 0;
    ...
    if (model) {
        if (state == Qt::Checked)
            CoinControlDialog::coinControl->destChange = CBitcoinAddress(
                    ui->lineEditCoinControlChange->text().toStdString()).Get();
        else
