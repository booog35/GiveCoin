        ...
        const CTxIn &txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end()) {
                ...
                if (txin.prevout.n >= wtx.vout.size())
                    ...
                    LogPrintf("WalletUpdateSpent found spent coin %s GIVE %s\n", FormatMoney(wtx.GetCredit()),
                              wtx.GetHash().ToString());
...
void CWalletTx::GetAccountAmounts(const string &strAccount, int64_t &nReceived,
    ...
    if (strAccount == strSentAccount) {
        BOOST_FOREACH(
        const PAIRTYPE(CTxDestination, int64_t) &s, listSent)
        nSent += s.second;
        nFee = allFee;
    }
    {
        LOCK(pwallet->cs_wallet);
        BOOST_FOREACH(
        const PAIRTYPE(CTxDestination, int64_t) &r, listReceived)
        {
    ...
    while (fRepeat) {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx) & item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex)) {
                ...
                if (fUpdated) {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s GIVE %s\n", FormatMoney(wtx.GetCredit()),
                              wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
...
bool CWallet::SelectCoins(int64_t nTargetValue, unsigned int nSpendTime,
    ...
    boost::function<bool(const CWallet *, int64_t, unsigned int, int, int, std::vector <COutput>,
                         ...
                         int64_t &)> f = &CWallet::SelectCoinsMinConf;
...
CWallet::CreateTransaction(const vector <pair<CScript, int64_t>> &vecSend, CWalletTx &wtxNew, CReserveKey &reservekey,
    ...
    int64_t nValue = 0;
    BOOST_FOREACH(
    const PAIRTYPE(CScript, int64_t) &s, vecSend)
    {
        if (nValue < 0)
            return false;
        nValue += s.second;
...
bool CWallet::CreateCoinStake(const CKeyStore &keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees,
    ...
    pcoin, setCoins)
    {
        ...
        for (unsigned int n = 0; n < min(nSearchInterval, (int64_t) nMaxStakeSearchInterval) && !fKernelFound &&
                                 pindexPrev == pindexBest; n++) {
            ...
            if (CheckKernel(pindexPrev, nBits, txNew.nTime - n, prevoutStake, &nBlockTime)) {
                ...
                uint64_t nTotalSize =
                        pcoin.first->vout[pcoin.second].nValue + GetProofOfStakeReward(pIndex0, nCoinAge, nFees);
    ...
    BOOST_FOREACH(CWalletTx * pcoin, vCoins)
    {
        ...
        for (unsigned int n = 0; n < pcoin->vout.size(); n++) {
            if (IsMine(pcoin->vout[n]) && pcoin->IsSpent(n) &&
                ...
                LogPrintf("FixSpentCoins found lost coin %s GIVE %s[%d], %s\n",
                ...
                if (!fCheckOnly) {
                    ...
                    pcoin->WriteToDisk();
                       ...
                       (txindex.vSpent.size() > n && !txindex.vSpent[n].IsNull())) {
                LogPrintf("FixSpentCoins found spent coin %s GIVE %s[%d], %s\n",
                          FormatMoney(pcoin->vout[n].nValue), pcoin->GetHash().ToString(), n,
                          fCheckOnly ? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
