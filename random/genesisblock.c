...
int main(int argc, char *argv[]) {
    ...
    pubkey_len = strlen(argv[1]) /
                 2; // One byte is represented as two hex characters, thus we divide by two to get real length.
    ...
    printf("\nCoinbase: %s\n\nPubkeyScript: %s\n\nMerkle Hash: %s\nByteswapped: %s\n", txScriptSig, pubScriptSig,
           merkleHash, merkleHashSwapped);
    ...
    //if(generateBlock)
    {
        ...
        while (1) {
            ...
            unsigned int check = *((uint32_t * )(
                    block_hash2 + 28)); // The hash is in little-endian, so we check the last 4 bytes.
